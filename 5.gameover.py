...
total_time = 100  # 총시간
start_ticks = pygame.time.get_ticks()  # 시작 시간 >> tick을 받아옴!!  while 밖에 위치
...
# 공 크기에 따른 최초 스피드
# 공이 튀겨졌을때 y값이 -가 되어야 위로 올라간다. 공이 작아질수록 덜 튕김
...
while running:
    ...
    # 경과 시간 계산(milisecond라 1000으로 나눠 1초로 표시)!! while 내부에 위치
...
# ★ 밖에서 해줘야함 아니면 Game over가 플레이 하는 내내 떠있음 
# 상황에 따른 메시지 띄우기(게임종료,타임오버,미션성공)
message = game_font.render(game_result, True,(255,255,0)) # 노란색
message_rect = message.get_rect( center = (int(screen_width/2), int(screen_height/2)) )  # 소수점제거 위해 int로 감쌈
screen.blit(message,message_rect)

pygame.display.update()  # 상황에 따라 메세지 띄우는게 있으니까 필요
